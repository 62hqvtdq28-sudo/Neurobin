...
document.addEventListener('click', function(e) {
  // iOS Safari fix: walk up DOM since e.target may be SVG icon
  var btn = e.target;
  while (btn && btn !== document.body) {
    if (btn.getAttribute && btn.getAttribute('data-action')) break;
    btn = btn.parentElement;
  }
  if (!btn || !btn.getAttribute || !btn.getAttribute('data-action')) return;
  ...
  if (action === 'delete-discount') {
    ...
    copyDiscountCode(btn.getAttribute('data-code'));
  } else if (action === 'toggle-discount') {
    var newState = btn.getAttribute('data-newstate') === 'true';
    toggleCodeActive(id, newState);
  } else if (action === 'open-usage-editor') {
    var maxUAttr = btn.getAttribute('data-maxu');
    var maxU = (maxUAttr !== '' && maxUAttr !== null) ? parseInt(maxUAttr) : null;
