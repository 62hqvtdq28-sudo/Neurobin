...
(function() {
  ...
  // ✅ Mobile RLS Fix: helper to verify session before any write operation
  // على الهاتف قد تنتهي الجلسة بصمت مما يسبب خطأ RLS — نتحقق أولاً
  async function _requireSession() {
    const { data: { session } } = await _db.auth.getSession();
    if (!session) throw new Error('انتهت جلستك — يرجى تسجيل الدخول مجدداً');
    return session;
  }
  ...
  async function ins(table, row) {
    await _requireSession(); // ✅ Mobile RLS Fix
  ...
  const ImageStorage = {
    async upload(file) {
      ...
      // iPad Safari يفشل أحياناً مع File objects في fetch() — Uint8Array أكثر موثوقية
      let payload = file;
      try {
  ...
  console.log('[SupaDB] v2.2 \u2713 (iPad Safari Fix + Mobile RLS Fix)');
})();
