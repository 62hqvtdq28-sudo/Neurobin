        ...
        <div class="p-2.5 product-card-body">
          ...
          <div class="flex items-center justify-between">
            <div class="flex flex-col leading-none">
              ...
              ${(product.qty2Price || product.qty3Price) ? `<div class="flex flex-wrap gap-1 mt-1">${product.qty2Price ? `<span class="text-xs text-brand-600 bg-brand-50 px-1.5 py-0.5 rounded-md whitespace-nowrap">٢ قطعة: ${SecurityValidator.escapeHtml(formatPrice(product.qty2Price))}</span>` : ''}${product.qty3Price ? `<span class="text-xs text-brand-600 bg-brand-50 px-1.5 py-0.5 rounded-md whitespace-nowrap">٣ قطع: ${SecurityValidator.escapeHtml(formatPrice(product.qty3Price))}</span>` : ''}</div>` : ''}
            </div>
