        ...
        <div class="p-2.5 product-card-body">
          ...
          <div class="flex items-center justify-between">
            <div class="flex flex-col leading-none">
              ...
              ${product.qty2Price ? `<span class="text-xs text-gray-400 line-through leading-none mt-1 mb-0.5">${SecurityValidator.escapeHtml(formatPrice(product.price * 2))}</span><span class="${product.originalPrice ? 'text-base' : 'text-sm'} font-bold text-red-600 leading-none">٢ قطعة: ${SecurityValidator.escapeHtml(formatPrice(product.qty2Price))}</span>` : ''}${product.qty3Price ? `<span class="text-xs text-gray-400 line-through leading-none mt-1 mb-0.5">${SecurityValidator.escapeHtml(formatPrice(product.price * 3))}</span><span class="${product.originalPrice ? 'text-base' : 'text-sm'} font-bold text-red-600 leading-none">٣ قطع: ${SecurityValidator.escapeHtml(formatPrice(product.qty3Price))}</span>` : ''}
            </div>
