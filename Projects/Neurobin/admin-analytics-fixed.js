/* admin-analytics-fixed.js — v4 (FIXED: Persistent deletion + Enhanced Profit Details)
   
   ROOT CAUSE OF ISSUE #1:
   - Analytics deletion was stored only in localStorage
   - loadAnalytics() had no check for Supabase deletion flag
   - Page refresh would reload analytics from orders (ignoring localStorage deletion)
   - Need to store deletion flag in Supabase for persistence across sessions

   SOLUTION:
   - Check Supabase settings.analytics_deleted_at timestamp
   - If set and recent, skip loading analytics
   - Store undo snapshot in Supabase for proper restoration
   - Clear all caches on deletion to prevent ghost data
*/
...
var _analyticsDeletedAt = null; // Track Supabase deletion flag

window._reloadAnalytics = function() {
  // Clear both localStorage AND Supabase deletion flag
...
async function loadAnalytics() {
  ...
  // STEP 1: Check if analytics were deleted in Supabase (persistent across all sessions)
  try {
    if (typeof SupaDB !== 'undefined' && SupaDB.Settings) {
      ...
      // If deletion flag exists and is less than 24 hours old, show deleted state
  ...
  try {
    ...
    // Apply reset date filter if exists
    try {
    ...
    // Load cost data and product names for profit calculations
    var _costMap = {};
    var _nameToId = {};
    try {
    ...
    var clsMap = {
      green:{card:'border-green-100 bg-green-50',card:'border-green-100 bg-green-50',icon:'bg-green-100 text-green-700'},
...
async function _loadProfitBreakdown() {
  ...
  try {
    ...
    // ISSUE #2-5 FIX: Enhanced profit calculations with complete details
    var _soldProducts = [];
    products
    ...
    // ISSUE #3 FIX: Profit ranking - highest profit first, missing costs at bottom
...
async function deleteAnalyticsView() {
  ...
  if (!_lastAnalyticsSnapshot) {
    if(typeof showToast==='function') showToast('لا توجد بيانات لحذفها','error');
    return;
  }
  
  if (!confirm('هل تريد مسح عرض الإحصائيات الحالية؟ يمكنك التراجع لاحقاً')) return;
  
  // ISSUE #1 FIX: Save deletion flag to Supabase for persistence
...
// Chart rendering functions (unchanged from original)
