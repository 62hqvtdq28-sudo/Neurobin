...
function setLoginError(msg) {
  ...
  // Increased to 30 seconds so user can see the error clearly
  setTimeout(() => { if (el) el.classList.add('hidden'); }, 30000);
}

function showLoginLockedMessage(remainingMs) {
  var mins = Math.ceil((remainingMs || 600000) / 60000);
  setLoginError('تم تجاوز عدد المحاولات. يرجى الانتظار ' + mins + ' دقيقة.');
}
...
async function logout(clearRemember) {
  ...
  var _ls = document.getElementById('loginScreen'); var _ad = document.getElementById('adminDashboard'); if (_ls) _ls.classList.remove('hidden'); if (_ad) _ad.classList.add('hidden');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  updateCurrentTime();
  // Auth state listener (safe here, SupaDB is guaranteed loaded)
  if (typeof SupaDB !== 'undefined' && SupaDB.Auth) {
    SupaDB.Auth.onStateChange(function(session) {
      if (!session) {
        var _ls = document.getElementById('loginScreen');
        var _ad = document.getElementById('adminDashboard');
        if (_ls) _ls.classList.remove('hidden');
        if (_ad) _ad.classList.add('hidden');
      }
    });
  }
});
