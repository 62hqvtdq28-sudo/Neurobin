...
function toggleMoProductPicker() {
  ...
  if (isHidden) {
    ...
    var searchEl = document.getElementById('moPickerSearch');
...
function renderMoProductGrid(products) {
  ...
  grid.innerHTML = products.map(function(p) {
    var pid  = escapeHTMLmo(String(p.id));
    var name = escapeHTMLmo(p.name_ar || p.name || '');
    ...
    var img = imgSrc
      ? '<img src="' + escapeHTMLmo(imgSrc) + '" class="w-full h-12 object-contain mb-1" onerror="this.style.display='none'">'
    ...
    return '<div onclick="addMoProduct('' + pid + '')" class="cursor-pointer rounded-xl p-2 text-center border-2 transition-all ' + border + '" title="' + name + '">'
      + img
...
function addMoProduct(productId) {
  ...
  var searchEl = document.getElementById('moPickerSearch');
  var q = searchEl ? searchEl.value.toLowerCase() : '';
  renderMoProductGrid(q
    ? _moProducts.filter(function(pr) { return (pr.name_ar || pr.name || '').toLowerCase().includes(q); })
    : _moProducts);
}
...
function renderMoSelectedProducts() {
  ...
  container.innerHTML = _moCartItems.map(function(item) {
    var pid = escapeHTMLmo(String(item.id));
    return '<div class="flex items-center gap-2 bg-white rounded-xl px-3 py-2 border border-brand-100">'
      + '<span class="flex-1 text-xs font-medium text-brand-800 truncate">' + escapeHTMLmo(item.name) + '</span>'
...
function updateManualTotal() {
  ...
  if (!document.querySelector('.mo-item-qty')) {
...
async function saveManualOrder() {
  ...
  // Sync live qty/price edits into _moCartItems
  document.querySelectorAll('.mo-item-qty').forEach(function(qtyEl) {
  ...
  _moCartItems.forEach(function(cartItem) {
    ...
    items.push({
      ...
      product_name: cartItem.name || 'منتج',
  ...
  try {
    var orderRes = await SupaDB._db.from('orders').insert({
      customer_name:    name,
    ...
    if (items.length > 0) {
      var itemsToInsert = items.map(function(item) {
        return Object.assign({ order_id: orderRes.data.id }, item);
      });
      var itemsRes = await SupaDB._db.from('order_items').insert(itemsToInsert);
      if (itemsRes.error) console.warn('Items insert warning:', itemsRes.error.message);
    }
