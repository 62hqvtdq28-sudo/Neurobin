...
// ===================================================
// الطلبات اليدوية — Manual Order (Grid Picker)
// ===================================================
var _moProducts = [];
var _moSelected = {}; // { productId: { product, qty } }
...
function openManualOrderModal() {
  ...
  updateManualTotal();
  // Hide picker
  document.getElementById('moProductPicker').classList.add('hidden');
  // Load products
  SupaDB.Products.list().then(function(list){
    _moProducts = list;
    renderMoProductGrid(_moProducts);
    lucide.createIcons();
  }).catch(function(e){ console.warn('Products load:', e); lucide.createIcons(); });
}
...
function toggleMoProductPicker() {
  ...
  if (!picker.classList.contains('hidden')) {
    renderMoProductGrid(_moProducts);
    lucide.createIcons();
    document.getElementById('moPickerSearch').focus();
  }
}

function filterMoProducts() {
  var q = (document.getElementById('moPickerSearch').value||'').toLowerCase().trim();
  var filtered = q ? _moProducts.filter(function(p){ return (p.name_ar||p.name||'').toLowerCase().includes(q); }) : _moProducts;
  renderMoProductGrid(filtered);
}

function renderMoProductGrid(list) {
  var grid = document.getElementById('moProductGrid');
  if (!list || list.length === 0) {
    grid.innerHTML = '<div class="col-span-5 text-center py-4 text-xs text-brand-400">لا توجد منتجات</div>';
    return;
  }
  grid.innerHTML = list.map(function(p) {
    var pid = String(p.id);
    var isSelected = !!_moSelected[pid];
    var img = p.image_url ? '<img src="'+escapeHTML(p.image_url)+'" class="w-full h-12 object-contain mb-1" loading="lazy">' 
                          : '<div class="w-full h-12 bg-brand-100 rounded flex items-center justify-center mb-1"><i data-lucide="package" class="w-5 h-5 text-brand-400"></i></div>';
    return '<div onclick="toggleMoProduct(''+pid+'')" class="mo-grid-card cursor-pointer rounded-lg p-1.5 text-center border-2 transition-all '+
      (isSelected ? 'border-brand-600 bg-brand-50' : 'border-transparent bg-white hover:border-brand-300')+'" data-pid="'+pid+'">' +
      img +
      '<p class="text-xs font-medium text-brand-900 leading-tight truncate">'+(p.name_ar||p.name||'')+'</p>' +
      '<p class="text-xs text-brand-600">'+(p.price||0).toLocaleString()+'</p>' +
      (isSelected ? '<div class="mt-0.5 text-xs bg-brand-600 text-white rounded-full px-1">✓</div>' : '') +
      '</div>';
  }).join('');
  lucide.createIcons();
}

function toggleMoProduct(pid) {
  pid = String(pid);
  var product = _moProducts.find(function(p){ return String(p.id)===pid; });
  if (!product) return;
  if (_moSelected[pid]) {
    delete _moSelected[pid];
  } else {
    _moSelected[pid] = { product: product, qty: 1 };
  }
...
function renderMoSelectedProducts() {
  ...
  var keys = Object.keys(_moSelected);
  if (keys.length === 0) {
    container.innerHTML = '<p class="text-xs text-brand-400 text-center py-3">لم يتم اختيار منتجات بعد</p>';
    return;
  }
  container.innerHTML = keys.map(function(pid) {
    var item = _moSelected[pid];
    var p = item.product;
    var img = p.image_url ? '<img src="'+escapeHTML(p.image_url)+'" class="w-10 h-10 object-contain rounded-lg border border-brand-100" loading="lazy">'
                          : '<div class="w-10 h-10 bg-brand-100 rounded-lg flex items-center justify-center"><i data-lucide="package" class="w-4 h-4 text-brand-400"></i></div>';
    return '<div class="flex items-center gap-3 bg-white rounded-xl p-3 border border-brand-100">' +
      img +
      '<div class="flex-grow min-w-0"><p class="text-sm font-semibold text-brand-900 truncate">'+(p.name_ar||p.name||'')+'</p>' +
      '<p class="text-xs text-brand-600">'+(p.price||0).toLocaleString()+' × '+item.qty+' = '+((p.price||0)*item.qty).toLocaleString()+' د.ع</p></div>' +
      '<div class="flex items-center gap-1 flex-shrink-0">' +
      '<button onclick="changeMoQty(''+pid+'',-1)" class="w-7 h-7 bg-brand-100 hover:bg-brand-200 rounded-full flex items-center justify-center text-brand-700 font-bold text-sm">-</button>' +
      '<span class="w-8 text-center font-bold text-brand-900 text-sm">'+item.qty+'</span>' +
      '<button onclick="changeMoQty(''+pid+'',1)" class="w-7 h-7 bg-brand-100 hover:bg-brand-200 rounded-full flex items-center justify-center text-brand-700 font-bold text-sm">+</button>' +
      '<button onclick="toggleMoProduct(''+pid+'')" class="w-7 h-7 bg-red-100 hover:bg-red-200 rounded-full flex items-center justify-center text-red-500 mr-1"><i data-lucide="trash-2" class="w-3.5 h-3.5"></i></button>' +
      '</div></div>';
  }).join('');
  lucide.createIcons();
}

function changeMoQty(pid, delta) {
  if (!_moSelected[pid]) return;
  _moSelected[pid].qty = Math.max(1, Math.min(99, (_moSelected[pid].qty||1) + delta));
  renderMoSelectedProducts();
  updateManualTotal();
}

function updateManualTotal() {
  var subtotal = Object.values(_moSelected).reduce(function(sum, item){
    return sum + (item.product.price||0) * (item.qty||1);
  }, 0);
...
async function saveManualOrder() {
  ...
  var selectedKeys = Object.keys(_moSelected);
  if (selectedKeys.length === 0) { showToast('يرجى اختيار منتج واحد على الأقل', 'error'); return; }

  var subtotal = 0;
  var items = selectedKeys.map(function(pid) {
    var item = _moSelected[pid];
    var lineTotal = (item.product.price||0) * (item.qty||1);
    subtotal += lineTotal;
    return { product_id: pid, product_name: item.product.name_ar||item.product.name||'', quantity: item.qty||1, price: item.product.price||0, subtotal: lineTotal };
  });
  ...
  try {
    var { data: order, error: orderErr } = await SupaDB._db.from('orders').insert({
      ...
      customer_address: address||null, notes: notes||null,
    ...
    if (items.length > 0) {
      var toInsert = items.map(function(item){ return Object.assign({ order_id: order.id }, item); });
      await SupaDB._db.from('order_items').insert(toInsert);
    }
