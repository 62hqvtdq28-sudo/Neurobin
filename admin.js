...
function saveQuickEdit(id) {
  ...
  var name = InputValidator.text(document.getElementById('qe-name-' + id).value.trim(), 200);
  var price = InputValidator.validatePrice(document.getElementById('qe-price-' + id).value);
  var stock = InputValidator.stock(document.getElementById('qe-stock-' + id).value);
...
async function saveProduct() {
  try {
    ...
    var name = InputValidator.text(document.getElementById('productName').value.trim(), 200);
    var price = InputValidator.validatePrice(document.getElementById('productPrice').value);
    var stock = InputValidator.stock(document.getElementById('productStock').value);

    if (!name) throw new Error('يرجى إدخال اسم المنتج');
    if (price === null) throw new Error('يرجى إدخال سعر صحيح');
