...
function openManualOrderModal() {
  ...
  document.getElementById('moProductRows').innerHTML = '';
  document.getElementById('moTotal').textContent = '0 د.ع';
  // Load products
  SupaDB.Products.list().then(function(list){
    _moProducts = list;
    addManualOrderRow(); // start with one row
    lucide.createIcons();
  }).catch(function(e){ console.warn('Products load error:', e); addManualOrderRow(); lucide.createIcons(); });
}
...
function updateManualTotal() {
  var rows = document.querySelectorAll('.mo-row');
  var subtotal = 0;
  rows.forEach(function(row){
    var qty = parseInt(row.querySelector('.mo-qty').value)||1;
  ...
  document.getElementById('moTotal').textContent = total.toLocaleString('ar-IQ') + ' د.ع';
}
...
async function saveManualOrder() {
  ...
  var rows = document.querySelectorAll('.mo-row');
  if (rows.length === 0) { showToast('يرجى إضافة منتج واحد على الأقل', 'error'); return; }

  var items = [];
  var subtotal = 0;
  var valid = true;
  rows.forEach(function(row){
    var select = row.querySelector('.mo-product-select');
    var qty = parseInt(row.querySelector('.mo-qty').value)||1;
    var price = parseFloat(row.querySelector('.mo-price').value)||0;
    var productId = select.value;
    var productName = select.options[select.selectedIndex] ? select.options[select.selectedIndex].text : 'منتج يدوي';
    if (!productId && !price) { valid = false; return; }
    var lineTotal = qty * price;
    subtotal += lineTotal;
    items.push({ product_id: productId||null, product_name: productName, quantity: qty, price: price, subtotal: lineTotal });
  });

  if (!valid || items.length === 0) { showToast('يرجى اختيار منتج وإدخال السعر', 'error'); return; }
